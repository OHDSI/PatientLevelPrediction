...
#' }
#' @export
setAdaBoost2 <- function(nEstimators = 50, learningRate = 1, seed = NULL) {
  ...
  result <- list(model = "fitAdaBoost2",
...
fitAdaBoost2 <- function(population,
                        plpData,
                        param,
                        search = "grid",
                        quiet = F,
  ...
  hyperParamSel <- lapply(param, function(x) do.call(trainAdaBoost2, listAppend(x, 
                                                                      list(train = TRUE, 
  ...
  finalModel <- do.call(trainAdaBoost2, listAppend(param[[bestInd]], 
                                         list(train = FALSE, 
...
trainAdaBoost2 <- function(population, plpData, nEstimators = 50, learningRate = 1, seed = NULL, train = TRUE, modelLocation=NULL, quiet=FALSE) {
