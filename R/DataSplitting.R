...
#' 
#' @details Splits cohort, covariate, and outcome data into random subsets, to be used
#' for validation.
#' 
...
#' @param splits           This can be either a single integer, in which case the data will be split up into equally sized parts. If a vector
#' is provided instead, these are interpreted as the relative sizes of each part.
#' 
#' @return A list with entries for each part. An entry itself is a list containing a cohortData, covariateData, and outcomeData object.
#' 
#' @export
splitData <- function(cohortData, covariateData, outcomeData, splits = 2){
  ...
  for (i in 1:length(splits)) {
    ...
    sampledOutcomeData <- list(outcomes = sampledOutcomes, 
