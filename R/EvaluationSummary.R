...
getEvaluationStatistics_binary <- function(prediction, evalColumn, ...) {
  ...
  for (evalType in evalTypes) {
    ...
    result <- rbind(
      result, 
      c(evalType, 'FP', cm.val[2])
    )
    result <- rbind(
      result, 
      c(evalType, 'FN', cm.val[3])
    )
    result <- rbind(
      result, 
    ...
    ParallelLogger::logInfo(sprintf('%-20s%.2f', 'FP: ', cm.val[2]))
    ParallelLogger::logInfo(sprintf('%-20s%.2f', 'FN: ', cm.val[3]))
...
optimizeThreshold <- function(prediction, outcomeCount) { #edit this
  prediction = prediction
