...
#' }
#' @export
setLRTorch <- function(w_decay=c(0.003, 0.005, 0.007), epochs=c(20, 50, 100), class_weight = 0){
  ...
  result <- list(model='fitLRTorch', param= expand.grid(w_decay=w_decay,
                                                         epochs=epochs, class_weight = ifelse(is.null(class_weight),'NULL', class_weight)),
                 name='LR Torch')
...
fitLRTorch <- function(population, plpData, param, search='grid', quiet=F,
  ...
  for(i in 1:nrow(param)){
    ...
    auc <- do.call(trainLRTorch,list(epochs=as.character(param$epochs[i]), w_decay = as.character(param$w_decay[i]), class_weight = as.character(param$class_weight[i]), 
                                      train = TRUE))
