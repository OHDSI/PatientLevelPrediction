...
#' }
#' @export
setLRTorch2 <- function(w_decay=c(0.0005, 0.005), epochs=c(20, 50, 100), seed=NULL, 
  ...
  result <- list(model='fitLRTorch2', param=split(expand.grid(w_decay=w_decay, epochs=epochs, 
...
fitLRTorch2 <- function(population, plpData, param, search='grid', quiet=F,
                        outcomeId, cohortId, ...){
  ...
  hyperParamSel <- lapply(param, function(x) do.call(trainLRTorch2, 
                                                     listAppend(x, 
                                                                list(plpData = pydata,
  ...
  finalModel <- do.call(trainLRTorch2, listAppend(param[[bestInd]], 
                                                  list(plpData = pydata,
                                                       population = pPopulation,
                                                       quiet = quiet,
                                                       train=F,
  ...
  attr(result, 'type') <- ifelse(param[[bestInd]]$autoencoder == T, 'pythonAuto' ,'python2')
...
trainLRTorch2 <- function(population, plpData, modelOutput, epochs=100, w_decay = 0.001, seed=0, class_weight = 0, train=TRUE, autoencoder = FALSE, vae =FALSE, quiet){
