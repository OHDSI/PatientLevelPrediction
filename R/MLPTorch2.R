...
#' }
#' @export
setMLPTorch2 <- function(size=c(500, 1000), w_decay=c(0.0005, 0.005), 
  ...
  result <- list(model='fitMLPTorch2', param=split(expand.grid(size=size, w_decay=w_decay,
...
fitMLPTorch2 <- function(population, plpData, param, search='grid', quiet=F,
                   outcomeId, cohortId, ...){
  ...
  hyperParamSel <- lapply(param, function(x) do.call(trainMLPTorch2, 
  ...
  finalModel <- do.call(trainMLPTorch2, listAppend(param[[bestInd]], 
                                                   list(plpData=pydata,
                                                             population = pPopulation,
                                                             train=FALSE,
                                                             modelOutput=outLoc,
...
trainMLPTorch2 <- function(population, plpData, modelOutput, size=200, epochs=100, w_decay = 0.001, seed=0, class_weight = 0, train=TRUE, 
