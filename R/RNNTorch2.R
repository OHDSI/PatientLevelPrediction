...
#' }
#' @export
setRNNTorch2 <- function(hidden_size=c(50, 100), epochs=c(20, 50), seed=0, class_weight = 0, type = 'RNN'){
  ...
  result <- list(model='fitRNNTorch2', param=split(expand.grid(hidden_size=hidden_size,
...
fitRNNTorch2 <- function(population, plpData, param, search='grid', quiet=F,
                        outcomeId, cohortId, ...){
  ...
  result <- toSparseTorchPython2(plpData,population, map=NULL, temporal=T)
  ...
  hyperParamSel <- lapply(param, function(x) do.call(trainRNNTorch2, listAppend(x, 
  ...
  finalModel <- do.call(trainRNNTorch2, listAppend(param[[bestInd]], 
                                                  list(plpData = pydata,
                                                       population = pPopulation,
                                                       train=FALSE,
                                                       modelOutput=outLoc)))
...
trainRNNTorch2 <- function(plpData, population, epochs=50, hidden_size = 100, seed=0, class_weight= 0, type = 'RNN', train=TRUE, modelOutput, quiet=F){
