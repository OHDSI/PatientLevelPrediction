...
runMultiplePlp <- function(
  ...
  for(i in 1:nrow(as.data.frame(dataSettings))){
    ...
    if(!dataExists){
      ...
      databaseDetails$evalmetric <- dataSettings$evalmetric[i] #new addition
  ...
  if(!onlyFetchData){
    for(i in 1:nrow(as.data.frame(settingstable))){
      ...
      if(dataExists){
        ...
        if(!analysisExists){
          plpData <- PatientLevelPrediction::loadPlpData(file.path(saveDirectory, settings$dataLocation)) #remove package calling
      ...
      if(dataExists){
        ...
        if(!analysisExists){
          ...
          runPlpSettings <- list(
            ...
            outcomeId = modelDesign$outcomeId, 
            evalmetric = modelDesign$evalmetric, #new addition
            analysisId = settings$analysisId,
            populationSettings = modelDesign$populationSettings,
            splitSettings = modelDesign$splitSettings,
            sampleSettings = modelDesign$sampleSettings,
...
createModelDesign <- function(
  targetId,
  outcomeId,
  evalmetric ='computeAuc', #new addition
  ...
  checkIsClass(outcomeId, c('numeric','integer')) #might add a check for evaluation metric
  
  #print('the creation of model design is executed')
...
convertToJson <-function(
  ...
  result <- data.frame(
    ...
    evalmetric = unlist(lapply(modelDesignList, function(x) x$evalmetric)), #new addition
