...
integrated; see the
[description](https://github.com/OHDSI/GIS/issues/345#issuecomment-2417261931)
...
We defined our `target` cohort within the sampling procedures when
creating a subset of the Tufts Synthetic Data, and we described this
process at length elsewhere. For the purposes of using the PLP package,
we formalized this group of individuals in the cohort table using a
simple cohort definition including all individuals with “any visit”, and
include that atlas-compatible json definition here for reference.

We also created our `outcome` cohort - in this case, those patients with
COPD or a conceptual descendant thereof - in Atlas and have shared the
json definition (we also included an equivalent SQL script).

#### Step 2: Create Generic PLP Lasso Logistic Regression Model in R

It is possible to create an R package that serves as a basis for a PLP
model using Atlas, but given Atlas does not yet support the GIS
extension, we have created this demo model directly using the PLP
package.

After configuring the environment (very important!) appropriately, we
...
After defining parameters, we created a database settings object and
launched the `runMultiplePLP` function to create a base `plpData`
object:
...
The labels sub-object within `plpData` contains per-individual data
elements like gender and age; we added an additional data element to
...
structure referenced by the PLP workflow. To do this, we were able to
follow the feature engineering vignette and create two functions,
...
We can then execute these functions to create training and test data
...
Note that if we plot the output model of the `GAM` fitting in the
`implementPollutants` function, we end up with a plot that aligns well
with our underlying relationship between Odds Ratio and PM2.5
concentration that we used to distribute our synthetic data by location:
