getProtocolSpecification <- function(){
  ...
  for(fun in protspec$function.){
    ...
    for(i in 1:length(values)){
      if(is.null(values[[i]])){values[[i]] <- 'no default'}
      if(class(values[[i]])%in%c('call','name')){values[[i]] <- as.character(values[[i]])}
    }
    defaultValue <- unlist(lapply(values, function(x) paste(x, collapse=' ') ))
    
    entry[[j]] <- list(functionName=protspec$function.[j], description = protspec$description[j], 
                  order = protspec$order[j], inputName=inputsName,inputDefaultValues = defaultValue)
  }
