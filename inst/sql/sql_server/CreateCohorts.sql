...
{@use_sample}?{From ( select * 
                from ( select *, 
                row_number() over (order by cast((CAST(person_id*month(cohort_start_date) AS BIGINT) % 123) as int) * cast((CAST(year(cohort_start_date)*day(cohort_start_date) AS BIGINT) % 123) as int)) rn
  } 
