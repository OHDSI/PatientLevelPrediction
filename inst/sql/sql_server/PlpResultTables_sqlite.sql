...
CREATE TABLE @my_schema.@string_to_appendcohorts (
    cohort_id INTEGER PRIMARY KEY AUTOINCREMENT,
	  atlas_id bigint,
    cohort_name VARCHAR(100) NOT NULL, -- why 100?
...
CREATE TABLE @my_schema.@string_to_appenddatabase_details (
    ...
    database_acronym VARCHAR(20) NOT NULL,
	  database_version int NOT NULL,
    database_description VARCHAR(1000) NOT NULL,
...
CREATE TABLE  @my_schema.@string_to_appendmodel_designs (
    ...
    FOREIGN KEY (cohort_id) REFERENCES @string_to_appendcohorts(cohort_id),
