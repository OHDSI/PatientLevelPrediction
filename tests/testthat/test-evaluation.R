...
devtools::load_all('C:/Users/jreps/Documents/GitHub/PatientLevelPrediction')

test_that("evaluation", {

  prediction <- data.frame(value= runif(100), outcomeCount =round(runif(100))  )
  
  # test AUC:::auc and pROC:roc
  writeLines('Testing AUC...')
  ...
  # test brierScore(prediction)
  writeLines('Testing brier score...')
  prediction$dummy <- 1
  ...
  writeLines(paste0('Scoring package brier score: ', brier.scoring, '- plp package: ', brier.plp))
  expect_that(as.double(brier.scoring), equals(brier.plp))
  
  # averagePrecision(prediction)
  writeLines('Testing average precision...')
  ...
  writeLines(paste0('Matrics ave prec: ', aveP.metrics, '- plp package: ', aveP.plp))
  expect_that(as.double(aveP.metrics), equals(aveP.plp))
  
  ##########################################################################
  # test quantiles(prediction) 
  writeLines('Testing quantiles...')
  ...
  # rocSparse(prediction)
  writeLines('Testing sparse ROC...')
