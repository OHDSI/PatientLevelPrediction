...
Our age spline feature function will create a new feature using the FeatureExtraction age covariate (covariateId 1002).  We will implement a restricted cubic spline that requires specifying the number of knots.
.  Therefore, the inputs for this are: 
 * `knots` an integer/double specifying the number of knots
 
```{r, echo = TRUE, eval=FALSE}
createAgeSpineSettings <- function(
                     knots = 5
                     ){
  ...
  attr(featureEngineeringSettings, "fun") <- "implementAgeSpineSettings"
...
We now need to create the 'implement' function `implementAgeSpineSettings()`
...
In our example, the `createAgeSpineSettings()` will return a list with 'knots'.  The featureEngineeringSettings therefore contains this.

```{r tidy=FALSE,eval=FALSE}
implementAgeSpineSettings <- function(trainData, featureEngineeringSettings){
