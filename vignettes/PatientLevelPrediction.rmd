...
To set python up:
* Close your RStudio
* Install python 3.7 using anaconda (https://www.anaconda.com/download) - anaconda should update you path variable with the python binary.
...
* If not set up correctly then:
  + Windows users: make sure your anaconda python binary is in the Path: go to my computer -> system properties -> advanced system settings Then at the bottom right you’ll see a button: Environmental Variables, clicking on that will enable you to edit the PATH variable to add the Anaconda location 
...
### Creating A PLP Python Vitual Environment
...
Windows users: simple run 
```{r eval=FALSE} 
PatientLevelPrediction::setPythonEnvironment(envname = 'PLP', envtype = 'conda') 
``` 
to use the PLP environment 

Mac/linux users: to make sure R uses this environment you need to edit your .Rprofile with the location of the python binary for the PLP environment.    Edit the .Rprofile by running: 
```{r eval=FALSE}
usethis::edit_r_profile()
``` 
and add  
